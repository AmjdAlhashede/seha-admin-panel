...
async function generateSickLeavePDF(leave) {
    ...
    y = 112;
    doc.setTextColor(...labelColor);
    doc.text('Birth Date:', col2X, y);
    doc.setTextColor(...valueColor);
    const birthDate = leave.birthDate ? new Date(leave.birthDate).toLocaleDateString('en-GB') : 'N/A';
    doc.text(birthDate, col2X + 25, y);
    ...
    y += 10;
    doc.setTextColor(...labelColor);
    doc.text('City:', col2X, y);
    doc.setTextColor(...valueColor);
    doc.text(leave.city || 'N/A', col2X + 25, y);
...
async function generateAllLeavesPDF(leaves) {
    ...
    doc.text('Job', 95, y + 7);
    doc.text('City', 130, y + 7);
    doc.text('Date', 155, y + 7);
    ...
    leaves.slice(0, 15).forEach((leave, index) => {
        ...
        doc.text((leave.job || 'N/A').substring(0, 15), 95, y);
        doc.text((leave.city || 'N/A').substring(0, 12), 130, y);
        doc.text(new Date(leave.startDate).toLocaleDateString('en-GB'), 155, y);
